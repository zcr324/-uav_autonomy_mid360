#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# XTDrone-style communication node (py3 移植，来源：racer_lidar/scripts/multirotor_communication.py)。
# 职责：/xtdrone/iris_N/* 命令 -> MAVROS setpoint_raw/local 60Hz 连续流 + ARM/模式切换服务。
# 修改：pyquaternion -> tf.transformations；其余逻辑保持原版。
...
class Communication(object):
    ...
    def __init__(self, vehicle_type, vehicle_id):
        ...
        try:
            self.set_param_srv.wait_for_service(timeout=30)
            self.set_param_srv("COM_RCL_EXCEPT", rcl_except)
        except Exception as e:
            rospy.logwarn(self.vehicle_type + '_' + self.vehicle_id + ": COM_RCL_EXCEPT set failed: %s", e)

        rospy.loginfo(self.vehicle_type + '_' + self.vehicle_id + ": communication initialized")

    def start(self):
        '''
        main ROS thread: 60Hz 连续 setpoint 流（PX4 offboard 必需）
        '''
        rate = rospy.Rate(60)
    ...
    def construct_target(self, x=0, y=0, z=0, vx=0, vy=0, vz=0, afx=0, afy=0, afz=0, yaw=0, yaw_rate=0):
        ...
        if self.motion_type == 0:
            target_raw_pose.type_mask = 0b001111111000  # 使用 position + yaw + yaw_rate
        if self.motion_type == 1:
            target_raw_pose.type_mask = 0b111111000111  # 使用 velocity + yaw_rate
    ...
    def cmd_pose_flu_callback(self, msg):
        self.coordinate_frame = 9  # FRAME_BODY_FLU
        self.motion_type = 0
    ...
    def cmd_pose_enu_callback(self, msg):
        self.coordinate_frame = 1  # FRAME_LOCAL_NED（XTDrone 约定：位置须已是 NED 系）
        self.motion_type = 0
        yaw = msg.orientation.x
        yaw_rate = msg.orientation.y
        self.target_motion = self.construct_target(x=msg.position.x, y=msg.position.y, z=msg.position.z, yaw=yaw, yaw_rate=yaw_rate)

    def cmd_vel_flu_callback(self, msg):
        self.hover_state_transition(msg.linear.x, msg.linear.y, msg.linear.z, msg.angular.z)
        if self.hover_flag == 0:
            self.coordinate_frame = 8  # FRAME_BODY_FLU
            self.motion_type = 1
    ...
    def cmd_callback(self, msg):
        if msg.data == self.last_cmd or msg.data == '' or msg.data == 'stop controlling':
            ...
            rospy.loginfo(self.vehicle_type + '_' + self.vehicle_id + ": Armed " + str(self.arm_state))
            ...
            rospy.loginfo(self.vehicle_type + '_' + self.vehicle_id + ": " + msg.data)
    ...
    def arm(self):
        try:
            ...
            rospy.logwarn(self.vehicle_type + '_' + self.vehicle_id + ": arm exception: %s", e)
        rospy.logwarn(self.vehicle_type + '_' + self.vehicle_id + ": arming failed!")
        return False
    ...
    def disarm(self):
        try:
            ...
            rospy.logwarn(self.vehicle_type + '_' + self.vehicle_id + ": disarm exception: %s", e)
        rospy.logwarn(self.vehicle_type + '_' + self.vehicle_id + ": disarming failed!")
        return False
    ...
    def hover(self):
        ...
        rospy.loginfo(self.vehicle_type + '_' + self.vehicle_id + ": " + self.flight_mode)
    ...
    def flight_mode_switch(self):
        ...
        try:
            if self.flightModeService(custom_mode=self.flight_mode):
                ...
                rospy.logwarn(self.vehicle_type + '_' + self.vehicle_id + ": " + self.flight_mode + " failed")
                return False
        except Exception as e:
            rospy.logwarn(self.vehicle_type + '_' + self.vehicle_id + ": set_mode exception: %s", e)
            return False
